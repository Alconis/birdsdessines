...
if($clean){
	...
	$directory = "/homez.44/alconis/birdsdessines.fr/bd/temp/";
...
<div style="float:right;"><a class="action" href="?view=<?=$view;?>&clean=1">Nettoyer les BDs temporaires</a></div>
<div id="titles">
	<a class="<?php if($view == 'last') echo 'selected'; ?>" href="http://www.birdsdessines.fr/moderation/?view=last">50 derni&egrave;res BDs</a><a class="<?php if($view == 'modo') echo 'selected'; ?>" href="http://www.birdsdessines.fr/moderation/?view=modo">File de mod&eacute;ration</a><a class="<?php if($view == 'bin') echo 'selected'; ?>" href="http://www.birdsdessines.fr/moderation/?view=bin">Corbeille</a><a class="<?php if($view == 'comments') echo 'selected'; ?>" href="http://www.birdsdessines.fr/moderation/?view=comments">Commentaires</a><a class="<?php if($view == 'users') echo 'selected'; ?>" href="http://www.birdsdessines.fr/moderation/?view=users">Utilisateurs</a>
</div> <!-- #mod #titles -->
...
while ( $query->have_posts() ) {
	...
	if($hidden == 1){
		echo '<div class="mod_hidden"><img alt="Cette BD est cach&eacute;e (bandeau)" src="/wp-content/themes/birds-snapshot/images/eye_exclamation.png"></div>';
	}
...
if($view != 'bin'){
?>
		<a class="action" href="?view=<?=$view;?>&detail=<?=$post_id;?>&<?=$hidden?'show':'hide';?>=<?=$post_id;?>"><?=$hidden?'Enlever':'Ajouter';?> le bandeau <img src="/wp-content/themes/birds-snapshot/images/eye_exclamation.png"></a><a class="action" href="?view=<?=$view;?>&detail=<?=$post_id;?>&alert=<?=$post_id;?>">Signaler la BD (+1 <img src="/wp-content/themes/birds-snapshot/images/error.png">)</a><a class="action" href="?view=<?=$view;?>&detail=<?=$post_id;?>&reset=<?=$post_id;?>">Annuler les signalements (<img src="/wp-content/themes/birds-snapshot/images/error.png"> =0)</a><a class="action" href="?view=<?=$view;?>&detail=<?=$post_id;?>&<?=$certified?'uncertify':'certify';?>=<?=$post_id;?>"><?=$certified?'Enlever':'Ajouter';?> le badge <img src="/wp-content/plugins/editeur-bd/editor/assets/icons/certified.png" style="width: 16px;height:16px;"></a>
