...
if($confirm == '1'){
	...
	// Retrieve skip mail author
	$skip_mail_author = $_POST['skipMailAuthor'];
	...
	if($skip_mail_author != '1'){
		...
		echo "Envoi mail auteur: ignor&eacute;<br/>";
	}
...
if ("malformed" == $moderation_type){
	...
	$body_msg .= "<p>Le contenu de la BD que vous avez publi&eacute;e a &eacute;t&eacute; signal&eacute; comme d&eacute;j&agrave; existant.";
	$body_msg .= " Aussi, afin de ne pas surcharger le site par trop de redondances, veillez &agrave; &ecirc;tre inédit en v&eacute;rifiant auparavant avec la fonction 'Chercher sur le site' qu'elle n'existe pas.</p>";
}else{
...
<form method="POST" action="?trash=<?=$trash;?>&confirm=1">
	...
	<input type="checkbox" name="skipMailAuthor" value="1"> Ne pas envoyer de mail &agrave; l'auteur
